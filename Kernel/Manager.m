...
Internal`PacletFindFile[ctxtOrFile_String] /; PacletManagerEnabled[] :=
    Which[
        ...
        !StringMatchQ[ctxtOrFile, "/" ~~ ___] && !StringMatchQ[ctxtOrFile, "\\" ~~ ___] && !StringMatchQ[ctxtOrFile, LetterCharacter~~":"~~___],
            ...
            *)
            Block[{parts = FileNameSplit[ctxtOrFile], pacletsFromMap, p, ext, path, paths, existingFile, file, base},
                If[Length[parts] > 1,
...
*)
getPathMap[] := If[AssociationQ[$pathMap], $pathMap, makePathMap[]]

makePathMap[] :=
    Module[{paclets, p, map, ctxts, pathBases},
        paclets = takeLatestEnabledVersionOfEachPaclet[
                     PCfindMatching["Extension" -> "Kernel" | "Application" | "Path", "Collections"->{"User", "Layout", "Legacy", "Extra"}]
                  ];
        ...
        $pathMap = Merge[Cases[Flatten[map], Literal[_String -> _Paclet]], Join]
    ]
