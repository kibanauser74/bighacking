...
(* :Copyright: Mathematica source code (c) 1999-2015, Wolfram Research, Inc. All rights reserved. *)
        ...
        {useProxy, httpProxyHost, httpProxyPort, httpsProxyHost, httpsProxyPort, ftpProxyHost, ftpProxyPort,
           socksProxyHost, socksProxyPort} =
                 {"UseProxy", "HTTPProxyHost", "HTTPProxyPort", "HTTPSProxyHost", "HTTPSProxyPort",
                       "FTPProxyHost", "FTPProxyPort", "SocksProxyHost", "SocksProxyPort"} /. $managerData;
        {"UseProxy" -> useProxy,
...
PacletSiteAdd[siteURL_String, name_String, opts:OptionsPattern[]] :=
    ...
    Module[{dataChanged, existingSite, isLocal, isPrepend},
        dataChanged = False;
        If[!(StringMatchQ[siteURL, "http:*"] || StringMatchQ[siteURL, "https:*"]|| StringMatchQ[siteURL, "file:*"]),
...
PacletSiteUpdate[siteURL_String, opts:OptionsPattern[]] :=
    Module[{task, async, siteRec, timeout, wasAborted, startTime},
        ...
        *)
        task = startPacletSiteUpdate[siteURL, opts];
        If[Head[task] === AsynchronousTaskObject,
            async = OptionValue[Asynchronous];
            If[async === False,
                (* The timeout is to prevent cases where the task never finishes. No known reason why that would happen. *)
                timeout = Which[# === Automatic, 12, NumberQ[#], #, True, Infinity]& @ OptionValue["Timeout"];
                wasAborted = False;
                startTime = AbsoluteTime[];
                CheckAbort[
                    While[MemberQ[AsynchronousTasks[], task] && Length[getTaskData[task]] == 4 && AbsoluteTime[] < startTime + timeout,
                        Quiet @ WaitAsynchronousTask[task, "Timeout" -> .1];
                    ],
                (* On abort: *)
                    wasAborted = True
                ];
                (* We can get here in three possible states: (1) The async task finished, (2) the WaitAsynchronousTask timed out, (3) the WaitAsynchronousTask was aborted.
                   For (2) and (3) we want to ensure that the task gets stopped and removed. No harm calling those functions in (1), but Quiet the messages.
                *)
                Quiet[
...
startPacletSiteUpdate[siteURL_String, opts:OptionsPattern[]] :=
    Module[{siteRec, interactive, async, tempFile, task},
...
finishPacletSiteUpdate[task_] :=
    Module[{siteURL, file, statusCodeOrErrorString, success, interactive, async, data, sitePaclets, errorMsg, taskData},
        ...
        freeTaskData[task];
        success = True;
        If[statusCodeOrErrorString === 200,
        ...
        (* else *)
            (* Status code != 200 or is error string. *)
